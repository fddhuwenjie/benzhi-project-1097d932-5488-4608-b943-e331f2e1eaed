...
func (s *Service) PreflightProfile(ctx context.Context, caseID string, cmd PreflightProfileCommand) (accessibility.ProfilePreflight, error) {
	...
	s.preflightMu.Lock()
	if cached, ok := s.preflightCache[caseID]; ok && cached.ExpectedRevision == a.Case.Revision {
		s.preflightMu.Unlock()
		return cloneProfilePreflight(cached), nil
	}
	s.preflightMu.Unlock()
	result := accessibility.PreflightProfile(a.Case.Revision, cmd.RulesetVersion, cmd.RuleCodes, cmd.BlockingSeverities)
	s.preflightMu.Lock()
	s.preflightCache[caseID] = cloneProfilePreflight(result)
	s.preflightMu.Unlock()
	return result, nil
}
