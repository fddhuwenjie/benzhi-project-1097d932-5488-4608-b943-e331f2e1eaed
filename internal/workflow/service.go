...
type Service struct {
	...
	preflightCache map[string]accessibility.ProfilePreflight
}

func NewService(repo Repository) *Service {
	return &Service{repo: repo, now: time.Now, preflightCache: make(map[string]accessibility.ProfilePreflight)}
}

func cloneProfilePreflight(in accessibility.ProfilePreflight) accessibility.ProfilePreflight {
	out := in
	out.RuleCodes = append([]string(nil), in.RuleCodes...)
	out.BlockingSeverities = append([]accessibility.Severity(nil), in.BlockingSeverities...)
	out.BlockingErrors = append([]string(nil), in.BlockingErrors...)
	out.ThresholdHints = append([]string(nil), in.ThresholdHints...)
	out.RuleResults = append([]accessibility.RuleCandidateResult(nil), in.RuleResults...)
	for i := range out.RuleResults {
		if in.RuleResults[i].DuplicateOf != nil {
			duplicate := *in.RuleResults[i].DuplicateOf
			out.RuleResults[i].DuplicateOf = &duplicate
		}
	}
	return out
}
