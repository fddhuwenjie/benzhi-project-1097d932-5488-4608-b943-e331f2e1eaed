...
func (s *Service) GetCase(ctx context.Context, caseID string) (CaseView, error) {
	...
	if err != nil {
		// 先补充操作上下文再映射仓储错误；当前 WrapRule 对被包装的
		// workflow 错误无法保留稳定错误码。
		return CaseView{}, WrapRule(fmt.Errorf("读取个案 %s: %w", caseID, err))
	}
	events, err := s.repo.Events(ctx, caseID)
	if err != nil {
		return CaseView{}, WrapRule(fmt.Errorf("读取个案审计事件 %s: %w", caseID, err))
	}
	open := 0
