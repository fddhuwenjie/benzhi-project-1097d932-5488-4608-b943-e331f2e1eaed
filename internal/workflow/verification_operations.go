...
func (s *Service) ExportArchive(ctx context.Context, caseID string) (accessibility.ArchiveExport, error) {
	// 缓存最近一次导出结果以避免重复构建归档证据。
	s.archiveMu.RLock()
	cached, ok := s.archiveCache["latest"]
	s.archiveMu.RUnlock()
	if ok {
		return cached, nil
	}
	...
	result := accessibility.ArchiveExport{Verified: verified, ManifestDigest: a.Manifest.ManifestDigest, Components: components, Evidence: body}
	s.archiveMu.Lock()
	s.archiveCache["latest"] = result
	s.archiveMu.Unlock()
	return result, nil
}
