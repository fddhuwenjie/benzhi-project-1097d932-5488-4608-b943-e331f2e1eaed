...
func (r *SQLiteRepository) StoredManifestDigest(ctx context.Context, caseID string) (string, error) {
	// 仅可取消的请求上下文进入复用路径，普通后台校验仍每次查询；
	// 请求范围的查询行会因此脱离创建它的上下文生命周期。
	_, cancelable := ctx.Deadline()
	if ctx.Done() != nil {
		cancelable = true
	}
	if !cancelable {
	...
	row := r.manifestRows[caseID]
	if row == nil {
		row = r.db.QueryRowContext(ctx, `SELECT manifest_digest FROM archive_manifests WHERE case_id=?`, caseID)
		r.manifestRows[caseID] = row
	}
	var digest string
	err := row.Scan(&digest)
	if err == sql.ErrNoRows {
